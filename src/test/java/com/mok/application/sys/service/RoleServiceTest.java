...
class RoleServiceTest {
    ...
    private final String TEST_ROLE_CODE = "admin_role";
    private final String TEST_DESCRIPTION = "Test admin role";
    ...
    void findPage_WithValidQuery_ReturnsRolePage() {
        ...
        when(roleRepository.findRolePage(any(PredicateSpecification.class), eq(pageable))).thenReturn(rolePage);
        
        Page<RoleDTO> result = roleService.findPage(query, pageable);
        
        assertNotNull(result);
        assertEquals(1, result.getContent().size());
        assertEquals(TEST_ROLE_NAME, result.getContent().get(0).getName());
        verify(roleRepository).findRolePage(any(PredicateSpecification.class), eq(pageable));
    }
    ...
    void updateState_WithValidNormalState_EnablesRole() {
        ...
        RoleDTO result = roleService.updateState(TEST_ROLE_ID, Const.RoleState.NORMAL);
    ...
    void updateState_WithValidDisabledState_DisablesRole() {
        ...
        RoleDTO result = roleService.updateState(TEST_ROLE_ID, Const.RoleState.DISABLED);
    ...
    void grant_WithMenuAndPermissionIds_GrantsSuccessfully() {
        ...
        verify(roleRepository).save(testRole);
        verify(redisCommands).del(Const.CacheKey.ROLE_PERMS + ":" + TEST_ROLE_ID);
    }
    ...
    void getRoleOptions_WithValidQuery_ReturnsRoleOptionList() {
        ...
        when(roleRepository.findAll(any(PredicateSpecification.class))).thenReturn(List.of(testRole));
        ...
        assertEquals(TEST_ROLE_ID, result.get(0).getId());
        assertEquals(TEST_ROLE_NAME, result.get(0).getName());
    ...
    void getRoleOptions_WithDisabledRole_ExcludesDisabledRoles() {
        ...
        when(roleRepository.findAll(any(PredicateSpecification.class))).thenReturn(List.of(disabledRole));
    ...
    void getPermissionsByRole_WithValidId_ReturnsPermissionDTOSet() {
        ...
        Set<PermissionDTO> result = roleService.getPermissionsByRole(TEST_ROLE_ID);
